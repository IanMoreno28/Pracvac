class VaccinesController < ApplicationController
  ...
  def index
    ...
    @vaccines = @vaccines.where(["nombre LIKE :filter ", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?
  ...
  def update
    respond_to do |format|
      if @vaccine.update(vaccine_params)
        format.html { redirect_to vaccines_path, notice: "Vaccine was successfully updated." }
  ...
  private
    ...
    def vaccine_params
      params.require(:vaccine).permit(:nombre)
    end
end
