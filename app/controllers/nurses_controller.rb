class NursesController < ApplicationController
  ...
  def index
    @nurses = Nurse.all
    @nurses = @nurses.order(:elemento).page(params[:page] || 1)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @nurse.save
        format.html { redirect_to nurses_url, notice: "Nurse was successfully created." }
  ...
  def update
    respond_to do |format|
      if @nurse.update(nurse_params)
        format.html { redirect_to nurses_url, notice: "Nurse was successfully updated." }
  ...
  private
    ...
    def nurse_params
      params.require(:nurse).permit(:nombre, :apellido, :user_id)
    end
end
